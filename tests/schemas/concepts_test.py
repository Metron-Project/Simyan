...
from simyan.resources import CONCEPT
...
def test_get_concept_fail(
    ...
    with Mocker(assert_all_requests_are_fired=True) as mock:
        url = f"https://comicvine.gamespot.mock/api{CONCEPT.singular_endpoint(id_=-1)}"
        mock.get(
            url=url,
...
def test_list_concepts(session: Comicvine) -> None:
    ...
    assert result.issue_count == 2744
