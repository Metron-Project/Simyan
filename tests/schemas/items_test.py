...
from simyan.resources import ITEM
...
def test_get_item(session: Comicvine) -> None:
    ...
    assert len(result.issues) == 3416
    assert len(result.story_arcs) == 460
    assert len(result.volumes) == 1041
...
def test_get_item_fail(
    ...
    with Mocker(assert_all_requests_are_fired=True) as mock:
        url = f"https://comicvine.gamespot.mock/api{ITEM.singular_endpoint(id_=-1)}"
        mock.get(
            url=url,
...
def test_list_items(session: Comicvine) -> None:
    ...
    assert result.issue_count == 3416
